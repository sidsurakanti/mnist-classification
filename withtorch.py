...
data_aug = transforms.Compose(
    [
        ToImage(),
        RandomAffine(degrees=10, translate=(0.15, 0.15), scale=(0.8, 1.1), shear=10),
        ToDtype(torch.float32, scale=True),
    ]
)

test_transforms = transforms.Compose([ToImage(), ToDtype(torch.float32, scale=True)])
...
train_DL = DataLoader(
    trainD, batch_size=BATCH_SIZE, shuffle=True, num_workers=8, pin_memory=True
)
test_DL = DataLoader(
    testD, batch_size=BATCH_SIZE, shuffle=True, num_workers=2, pin_memory=True
)
...
def train(dataloader, model, loss_fn, optimizer=None):
    ...
    for batch_i, (X, y) in enumerate(dataloader):  # in batches from DL
        ...
        if (batch_i) % 100 < 1:
            ...
            print(
                f"Curr: {batch_i*BATCH_SIZE + len(X)}/{size}, Loss = {loss.item():.4f}"
            )
